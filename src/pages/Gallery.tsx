...
const Gallery = () => {
  ...
  useEffect(() => {
    if (isApiConfigured()) {
      fetchGallery().then(setGalleryItems).catch(() => setGalleryItems(fallbackGallery));
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-24 pb-20 px-4">
        <div className="container mx-auto">
          ...
          <div className="columns-2 md:columns-3 lg:columns-4 gap-4 space-y-4">
            {filtered.map((item, i) => (
              <button key={item.id} onClick={() => setLightboxIndex(i)} className="group block w-full overflow-hidden rounded-lg break-inside-avoid">
      ...
      {lightboxIndex >= 0 && (
        <LightboxModal images={images} currentIndex={lightboxIndex} onClose={() => setLightboxIndex(-1)}
