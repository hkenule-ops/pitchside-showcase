...
import { ArrowLeft, Play, FileText, Target, Users, Trophy, Star, Loader2 } from "lucide-react";
...
const PlayerProfile = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const players = isApiConfigured() ? await fetchPlayers() : fallbackPlayers;
        setPlayer(players.find((p) => p.id === id) || null);
      } catch {
        setPlayer(fallbackPlayers.find((p) => p.id === id) || null);
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="flex items-center justify-center pt-32">
          <div className="text-center">
            <p className="text-foreground text-xl mb-4">Player not found</p>
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div className="relative">
              ...
              {player.videoUrl && (
                <button onClick={() => setShowVideo(true)} className="absolute bottom-6 right-6 w-16 h-16 bg-gradient-primary rounded-full flex items-center justify-center shadow-glow hover:scale-110 transition-transform">
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className="bg-gradient-primary text-primary-foreground px-4 py-1 rounded-full text-sm font-bold">#{player.jersey}</span>
              ...
              <div className="flex flex-wrap gap-4 mt-8">
                {player.videoUrl && (
                  <button onClick={() => setShowVideo(true)} className="px-6 py-3 bg-gradient-primary text-primary-foreground font-display uppercase tracking-wider rounded-lg shadow-glow hover:scale-105 transition-transform flex items-center gap-2">
                    <Play size={18} /> Watch Highlights
                  </button>
                )}
                {player.resumeUrl && (
                  <button onClick={() => setShowResume(true)} className="px-6 py-3 border-2 border-accent text-accent font-display uppercase tracking-wider rounded-lg hover:bg-accent hover:text-accent-foreground transition-colors flex items-center gap-2">
                ...
                <div className="mt-10">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {player.images.map((img, i) => (
                      <button key={i} onClick={() => setLightboxIndex(i)} className="aspect-square overflow-hidden rounded-lg group">
      ...
      {showVideo && player.videoUrl && <VideoModal videoUrl={player.videoUrl} onClose={() => setShowVideo(false)} />}
      {showResume && player.resumeUrl && <ResumeViewer url={player.resumeUrl} playerName={player.name} onClose={() => setShowResume(false)} />}
      {lightboxIndex >= 0 && (
        <LightboxModal images={player.images} currentIndex={lightboxIndex} onClose={() => setLightboxIndex(-1)}
