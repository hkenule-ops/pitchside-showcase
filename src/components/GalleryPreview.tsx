...
import { fetchGallery, isApiConfigured } from "@/lib/api";
import { fallbackGallery, type GalleryItem } from "@/lib/data";
import { Camera } from "lucide-react";

const GalleryPreview = () => {
  const [items, setItems] = useState<GalleryItem[]>(fallbackGallery.slice(0, 4));

  useEffect(() => {
    if (isApiConfigured()) {
      fetchGallery()
        .then((g) => setItems(g.slice(0, 4)))
        .catch(() => setItems(fallbackGallery.slice(0, 4)));
    }
  }, []);
    ...
    <section className="py-20 px-4 bg-secondary">
      <div className="container mx-auto">
        ...
        <div className="text-center mt-10">
          <Link to="/gallery" className="inline-block px-8 py-3 border-2 border-primary text-primary font-display text-sm uppercase tracking-wider rounded-lg hover:bg-primary hover:text-primary-foreground transition-colors">
            View Full Gallery
          </Link>
        </div>
      </div>
