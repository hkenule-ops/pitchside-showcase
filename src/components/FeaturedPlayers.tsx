...
const FeaturedPlayers = () => {
  ...
  useEffect(() => {
    if (isApiConfigured()) {
      fetchPlayers()
        .then((p) => setPlayers(p.filter((pl) => pl.featured).slice(0, 3)))
        .catch(() => setPlayers(fallbackPlayers.filter((p) => p.featured)));
    } else {
      setPlayers(fallbackPlayers.filter((p) => p.featured));
    }
  }, []);
    ...
    <section className="py-20 px-4 bg-background">
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {players.map((player) => (
            <Link key={player.id} to={`/players/${player.id}`} className="group relative overflow-hidden rounded-xl shadow-card">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                <div className="flex items-center gap-2 mb-2">
                  <span className="bg-gradient-primary text-primary-foreground px-3 py-1 rounded-full text-xs font-semibold uppercase">#{player.jersey}</span>
